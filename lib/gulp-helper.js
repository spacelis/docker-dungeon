var _, map, docker, utils, minimist;
  ...
  return {
    commonTasks: function(opts){
      ...
      gulp.task('rmi', 'Clean up untagged images', function(){
        return docker.nonTaggedImages().pipe(map.obj(function(img){
          return docker.rmImage(img);
        }));
      });
      gulp.task('clean', 'Remove all built images', function(){
        return docker.dockerfileFinder(all_image + "/Dockerfile").pipe(docker.imageTagger()).pipe(map.obj(function(tag){
          docker.rmImage(tag);
          return tag;
        }));
      });
      gulp.task('push', 'Push all built images to gladys', function(){
        ...
        img = argv.i || all_image;
        return docker.dockerfileFinder(img + "/Dockerfile").pipe(docker.imageTagger()).pipe(map.obj(function(tag){
          return docker.pushImage(tag);
        }));
      });
      gulp.task('rm', 'Removing all stopped containers', function(){
        return docker.stoppedContainers().pipe(map.obj(function(name){
