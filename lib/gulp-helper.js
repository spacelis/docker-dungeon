  ...
  return {
    ...
    certificateTasks: function(){
      return gulp.task('mkcert', 'Make a self-signed certificate for HTTPS connection', function(){
        utils.Shell.interactiveLauncher('openssl', ['genrsa', '-des3', '-passout', 'pass:x', '-out', 'server.pass.key', '2048']);
        utils.Shell.interactiveLauncher('openssl', ['rsa', '-passin', 'pass:x', '-in', 'server.pass.key', '-out', 'server.key']);
        utils.Shell.interactiveLauncher('rm', ['server.pass.key']);
        utils.Shell.interactiveLauncher('openssl', ['req', '-new', '-key', 'server.key', '-out', 'server.csr']);
        return utils.Shell.interactiveLauncher('openssl', ['x509', '-req', '-days', '365', '-in', 'server.csr', '-signkey', 'server.key', '-out', 'server.crt']);
      });
    },
    docTasks: function(){
      var md, gmdcss;
